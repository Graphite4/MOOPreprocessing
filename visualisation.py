...
# VISUALIZATIONS_PATH = 'visualisations'
ALPHA = 0.9
...
def visualize(X, y, X_test=None, y_test=None, appended=None, gamma=None, radii=None,
    ...
    if regions_center is not None and regions_radius is not None and gamma is not None:
        ...
        for x_i in np.linspace(regions_center[0] - regions_radius, regions_center[0] + regions_radius, REGIONS_STEPS):
            for y_i in np.linspace(regions_center[1] - regions_radius, regions_center[1] + regions_radius, REGIONS_STEPS):
                ...
                if algorithm.distance(p_i, regions_center) <= regions_radius:
                    pp.append(p_i)
    ...
    s=MARKER_SIZE, c="#CE6EF1", linewidths=LINE_WIDTH,
    alpha=ALPHA, marker=MARKER_SYMBOL, edgecolors=ORIGINAL_EDGE_COLOR
    )
    ...
    if file_name is not None:
        ...
        try:
            ...
            pass
        plt.savefig(os.path.join(file_name, "visualisation.pdf"), bbox_inches='tight')
        plt.close(fig)
    else:
        plt.show()


def prepare_data(X, y, n_minority_samples=20, scaler='MinMax'):
